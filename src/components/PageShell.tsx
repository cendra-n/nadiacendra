          ...
          <nav className="flex flex-wrap items-center gap-2 md:gap-2.5">
            ...
            {sections.map((s) => (
              <Link
                key={s.to}
                to={s.to}
                className="inline-flex items-center rounded-full border border-foreground/20 px-3.5 py-1.5 font-mono text-[11px] uppercase tracking-[0.16em] text-foreground/80 transition-colors hover:border-primary hover:text-primary"
              >
                {s.label}
              </Link>
            ))}
